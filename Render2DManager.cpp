...
void Render2DManager::TextDraw(string str, D3DXVECTOR2 pos, float size, bool Center, D3DCOLOR color)
{
	...
	if (!Center)
	{
		D3DXCreateFontA(g_Device, size, 0, 10, 1, FALSE, DEFAULT_CHARSET, 0, 0, 0, "Comic Sans Ms", &Font);
		D3DXMatrixTranslation(&mat, pos.x, pos.y, 0);
	}
	else
	{
		D3DXCreateFontA(g_Device, size, 0, 0, 1, FALSE, DEFAULT_CHARSET, 0, 0, 0, "Comic Sans Ms", &Font);
